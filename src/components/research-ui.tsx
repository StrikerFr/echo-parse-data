...
export function DataWall() {
 const source = (id: string) => sourceById(id);
 return <div className="grid border-l border-t border-border md:grid-cols-2 xl:grid-cols-3">{findings.map((f,i)=><article key={`${f.value}-${f.label}`} className="group flex min-h-[330px] flex-col border-b border-r border-border p-6 lg:p-8"><div className="flex justify-between gap-4 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground"><span>IND / {String(i+1).padStart(2,"0")}</span><span className="text-right">{f.year}</span></div><p className="mt-10 font-mono text-5xl font-semibold leading-none text-primary sm:text-6xl">{f.value}</p><p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-accent-foreground">{f.label}</p><p className="mt-5 line-clamp-3 text-xs leading-relaxed text-muted-foreground">{f.context}</p><div className="mt-auto flex items-end justify-between gap-4 pt-8"><span className="max-w-40 text-[10px] font-bold uppercase leading-relaxed text-muted-foreground">{source(f.sourceId)?.publisher}</span><SourceLink id={f.sourceId} label="Data link"/></div></article>)}</div>
}
